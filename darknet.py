...
def performDetect(image, thresh=0.25, configPath="darknet/cfg/yolov3.cfg", weightPath="darknet/yolov3.weights",
                  metaPath="darknet/cfg/coco.data", showImage=True, makeImageOnly=False, initOnly=False):
    """
    ...
    # Do the detection
    detections = detect(netMain, metaMain, image, thresh)  # if is used cv2.imread(image)
    # reformat detection result

    # raw_boxes = np.array(raw_boxes)
    # raw_boxes = raw_boxes[:, np.array([1, 0, 3, 2])].astype('int32')
    # height, width = image.shape[0], image.shape[1]
    # h_ratio = height / lib.network_height(netMain)
    # w_ratio = width / lib.network_width(netMain)

    raw_boxes, scores, classes = [], [], []
    for res in detections:
        classes.append(res[0])
        scores.append(res[1])
        box = [int(res[2][0] - res[2][2] / 2), int(res[2][1] - res[2][3] / 2),
               int(res[2][0] + res[2][2] / 2), int(res[2][1] + res[2][3] / 2)]
        # box = [int(num) for num in res[2]]
        # box[2] += box[0]
        # box[3] += box[1]
        raw_boxes.append(box)

    return raw_boxes, scores, classes
...
if __name__ == "__main__":
    ...
    raw_boxes, scores, classes = performDetect(img, configPath="./cfg/yolov3.cfg", weightPath="./yolov3.weights",
